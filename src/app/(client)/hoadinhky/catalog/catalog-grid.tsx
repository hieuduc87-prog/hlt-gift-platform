...
function ComboCard({ combo }: { combo: GiftComboTier }) {
  return (
    <Card variant="hover" padding="sm" className="flex flex-col overflow-hidden">
      {/* Image */}
      <Link href={`/hoadinhky/catalog/${combo.slug}`} className="block">
      ...
      <div className="flex flex-col flex-1 p-3 pt-3">
        ...
        {/* Name & Price */}
        <Link href={`/hoadinhky/catalog/${combo.slug}`} className="group">
        ...
        {/* CTA */}
        <Link
          href={`/hoadinhky/orders/new?combo=${combo.slug}`}
